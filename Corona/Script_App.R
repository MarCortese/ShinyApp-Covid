...
frow7 <- fluidRow(
  radioButtons("variabile", label = h3("Scegli variabile"),
               choices = list("Positivi" = "pos", "Guariti" = "gu", "Decessi" = "dec"), 
               selected = "pos"),
...
server <- function(input, output,session) { 
  ...
  d_reg$totale_attualmente_positivi<-as.numeric(as.character(d_reg$totale_attualmente_positivi))
  d_reg$nuovi_attualmente_positivi<-as.numeric(as.character(d_reg$nuovi_attualmente_positivi))
  ...
  sceltaplot<-reactive({
    if(as.character(input$variabile)=="pos"){
      plot_ly(d_naz, x = d_naz$data, y = d_naz$nuovi_attualmente_positivi, type = 'bar', text= paste0("Nuovi positivi: ",d_naz$nuovi_attualmente_positivi),
